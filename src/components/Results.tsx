...
import { Award, Search, User, Calendar, BookOpen, ChevronRight } from 'lucide-react';
...
export default function Results({ user, role, userProfile }: { user: any, role: string | null, userProfile: any }) {
  ...
  const [viewMode, setViewMode] = useState<'personal' | 'leaderboard'>(user ? 'personal' : 'leaderboard');
  ...
  const filteredResults = results.filter(result => {
    ...
    const isPersonal = effectiveEmail && result.studentEmail.toLowerCase() === effectiveEmail.toLowerCase();
    
    if (role === 'admin') return matchesSearch && matchesClass && matchesSubject;
    if (viewMode === 'personal') return isPersonal && matchesClass && matchesSubject;
        ...
        >
          <div className="flex flex-col md:flex-row items-center gap-4">
             ...
             <div className="flex-1">
                <p className="text-xs font-black uppercase text-slate-400 tracking-widest mb-1">Quick Result Lookup</p>
                <input 
                  type="email"
                  placeholder="Enter your email to find marks..."
                  value={guestEmail}
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="text-2xl font-black text-slate-800">{result.subject}</h3>
                    ...
                    <div className="flex items-center space-x-6 text-sm text-slate-400 font-bold">
                      <span className="text-slate-800 uppercase text-xs">{result.studentName}</span>
